...
export async function findAvailableSlots(urgency, count = 3, preferredDay = null) {
  ...
  const SLOT_END_HOUR = 23;             // 5 PM MST = 23:00 UTC
  ...
  switch (urgency) {
    case "immediate":
      // Next available, earliest first — look ahead enough to always find 3+ slots
      searchEndDate.setDate(searchEndDate.getDate() + 10);
      break;
    case "soon":
  ...
  while (currentDate <= searchEndDate && slots.length < count * 3) {
    ...
    // Slot times in UTC that correspond to Mountain Time
    // 15:00 UTC = 9:00 AM MST, 17:30 UTC = 11:30 AM MST, 20:00 UTC = 2:00 PM MST
