...
app.post("/api/hr-demo", async (req, res) => {
  try {
    const { message, sessionId } = req.body;
    const fakePhone = "+1HR" + (sessionId || "000000").replace(/[^0-9]/g, "").slice(0, 9).padEnd(9, "0");
    const aiResponse = await handleHRMessage({ from: fakePhone, body: message, messageSid: "HRDEMO_" + Date.now() });
    res.json({ reply: aiResponse.text, stage: aiResponse.stage, qualified: aiResponse.qualified, applicantData: aiResponse.applicantData || {} });
  } catch (error) {
    console.error("HR Demo error:", error);
    res.status(500).json({ reply: "Something went wrong. Try again!", error: error.message });
  }
});
...
app.post("/api/hr-demo/reset", (req, res) => {
  const { sessionId } = req.body;
  const fakePhone = "+1HR" + (sessionId || "000000").replace(/[^0-9]/g, "").slice(0, 9).padEnd(9, "0");
  try { db.prepare("DELETE FROM hr_messages WHERE phone = ?").run(fakePhone); } catch(e) {}
  try { db.prepare("DELETE FROM hr_conversations WHERE phone = ?").run(fakePhone); } catch(e) {}
  try { db.prepare("DELETE FROM applicants WHERE phone = ?").run(fakePhone); } catch(e) {}
  res.json({ status: "reset" });
});
...
app.get("/api/applicants", (req, res) => {
  try {
    const applicants = db.prepare("SELECT * FROM applicants ORDER BY created_at DESC LIMIT 50").all();
    res.json(applicants);
  } catch(e) { res.json([]); }
});
...
// ─── Lily HR Demo Page ───
app.get("/hr-demo", (req, res) => {
  res.send(`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Lily HR Demo — Visiting Angels</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #e8f5e9; display: flex; justify-content: center; align-items: center; min-height: 100vh; padding: 20px; }
  .container { width: 100%; max-width: 420px; }
  .header { background: #2D8A5E; border-radius: 16px 16px 0 0; padding: 20px; text-align: center; }
  .header h1 { color: #fff; font-family: Georgia, serif; font-size: 18px; margin-bottom: 4px; }
  .header p { color: #c5e8d5; font-size: 13px; }
  .badge { display: inline-block; background: #fff; color: #2D8A5E; padding: 3px 12px; border-radius: 12px; font-size: 11px; font-weight: 700; margin-top: 8px; }
  .chat { background: #fff; height: 420px; overflow-y: auto; padding: 16px; display: flex; flex-direction: column; gap: 8px; }
  .msg { max-width: 82%; padding: 10px 14px; border-radius: 16px; font-size: 14px; line-height: 1.45; white-space: pre-line; animation: fadeIn 0.3s ease; }
  .msg.ai { align-self: flex-start; background: #e8f5e9; color: #1a2744; border-bottom-left-radius: 4px; }
  .msg.user { align-self: flex-end; background: #2D8A5E; color: #fff; border-bottom-right-radius: 4px; }
  .msg.system { align-self: center; background: transparent; color: #999; font-size: 12px; font-style: italic; text-align: center; }
  .typing { align-self: flex-start; background: #e8f5e9; padding: 12px 20px; border-radius: 16px; border-bottom-left-radius: 4px; display: none; }
  .typing span { display: inline-block; width: 8px; height: 8px; background: #aaa; border-radius: 50%; margin: 0 2px; animation: pulse 1.2s ease-in-out infinite; }
  .typing span:nth-child(2) { animation-delay: 0.2s; }
  .typing span:nth-child(3) { animation-delay: 0.4s; }
  .input-area { background: #fff; padding: 12px; display: flex; gap: 8px; border-top: 1px solid #e0e0e0; border-radius: 0 0 16px 16px; }
  .input-area input { flex: 1; padding: 10px 16px; border-radius: 24px; border: 1px solid #e0e0e0; font-size: 14px; outline: none; background: #f9f9f9; }
  .input-area input:focus { border-color: #2D8A5E; }
  .input-area button { width: 40px; height: 40px; border-radius: 50%; background: #2D8A5E; border: none; cursor: pointer; color: #fff; font-size: 16px; }
  .suggestions { display: flex; flex-wrap: wrap; gap: 6px; padding: 10px 0; }
  .suggestions button { padding: 6px 14px; border-radius: 16px; font-size: 12px; background: #e8f5e9; color: #2D8A5E; border: 1px solid #2D8A5E; cursor: pointer; }
  .suggestions button:hover { background: #2D8A5E; color: #fff; }
  .controls { display: flex; gap: 8px; margin-top: 10px; justify-content: center; }
  .controls button { padding: 8px 16px; border-radius: 8px; font-size: 12px; cursor: pointer; border: 1px solid #ddd; background: #fff; color: #666; }
  @keyframes fadeIn { from { opacity: 0; transform: translateY(6px); } to { opacity: 1; transform: translateY(0); } }
  @keyframes pulse { 0%, 100% { opacity: 0.3; transform: scale(0.8); } 50% { opacity: 1; transform: scale(1); } }
...
<div class="container">
  <div class="header">
    <h1>Meet Lily — HR Demo</h1>
    <p>Virtual HR Assistant for Caregiver Recruitment</p>
    <div class="badge">SANDBOX MODE</div>
  </div>
  <div class="chat" id="chat">
    <div class="msg system">Text Lily to start — try "Hi, I'm interested in a caregiver position"</div>
  </div>
  <div class="typing" id="typing"><span></span><span></span><span></span></div>
  <div class="input-area">
    <input type="text" id="input" placeholder="Type a message..." autocomplete="off">
  ...
  <div class="suggestions">
    <button onclick="useSuggestion(this)">Hi, I'm looking for a caregiver job</button>
    <button onclick="useSuggestion(this)">How much does it pay?</button>
    <button onclick="useSuggestion(this)">Do I need a CNA?</button>
    <button onclick="useSuggestion(this)">What shifts are available?</button>
  </div>
  <div class="controls">
    <button onclick="resetChat()">Reset Conversation</button>
    <button onclick="window.location='/demo'">Switch to Lead Demo</button>
  </div>
</div>
<script>
const sessionId = "HR" + Math.random().toString(36).slice(2, 9);
const chat = document.getElementById("chat");
const input = document.getElementById("input");
const typing = document.getElementById("typing");
input.addEventListener("keydown", e => { if (e.key === "Enter" && input.value.trim()) send(); });
...
async function send() {
  ...
  document.getElementById("sendBtn").disabled = true;
  try {
    const res = await fetch("/api/hr-demo", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: text, sessionId }) });
    const data = await res.json(); typing.style.display = "none"; addMsg(data.reply, "ai");
    if (data.qualified) addMsg("Applicant qualified! Interview booking offered.", "system");
  } catch (err) { typing.style.display = "none"; addMsg("Error connecting. Try again.", "system"); }
  document.getElementById("sendBtn").disabled = false; input.focus();
}
async function resetChat() {
  await fetch("/api/hr-demo/reset", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({ sessionId }) });
  chat.innerHTML = '<div class="msg system">Conversation reset — send a message to start again</div>';
}
</script>
</body>
