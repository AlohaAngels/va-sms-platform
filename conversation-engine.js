...
export async function handleInboundMessage({ from, body, messageSid }) {
  ...
  // If already booked, just let Claude answer questions — never re-offer booking
  if (conversation.stage === "assessment_booked") {
    const messageHistory = buildMessageHistory(from);
    const stateContext = buildStateContext(conversation, { score: "neutral", override: false, summary: "" });
  ...
  // Only offer calendar if qualified AND has address AND not already booked or selecting
  if (parsed.qualified && parsed.leadData.address && !["assessment_booked", "selecting_slot"].includes(parsed.stage)) return await offerCalendarSlots(from, parsed);
  if (parsed.qualified) { saveLead(from, parsed.leadData); syncLeadToHubSpot(from, parsed.leadData).catch(console.error); try { scheduleFollowUps(from, parsed.leadData); scheduleTextFollowUps(from, parsed.leadData); } catch(e) {} }
...
async function handleSlotSelection(from, body, conversation) {
  ...
  if (t.includes("1") || t.includes("first")) idx = 0;
  ...
  else if (t.includes("none") || t.includes("different") || t.includes("other") || t.includes("monday") || t.includes("tuesday") || t.includes("wednesday") || t.includes("thursday") || t.includes("friday") || t.includes("next week") || t.includes("what about") || t.includes("another") || t.includes("later")) {
    // Detect if they asked for a specific day
    const dayMap = { monday: 1, tuesday: 2, wednesday: 3, thursday: 4, friday: 5 };
    let preferredDay = null;
    for (const [name, num] of Object.entries(dayMap)) {
      if (t.includes(name)) { preferredDay = num; break; }
    }

    // Pull slots — filtered to specific day if requested
    ...
    if (ns && ns.length) {
      ...
      let r;
      if (dayName) {
        r = "Here are the " + dayName.charAt(0).toUpperCase() + dayName.slice(1) + " openings:\n\n" + formatSlotsForSMS(ns) + "\n\nWhich works best?";
      } else {
        r = "No problem! Here are other openings:\n\n" + formatSlotsForSMS(ns) + "\n\nDo any of these work?";
      }
      logMessage(from, "assistant", r, null);
      return { text: r, stage: "selecting_slot", qualified: true, leadData };
    } else {
      // No slots found for that day
      const dayName = preferredDay ? Object.keys(dayMap).find(k => dayMap[k] === preferredDay) : null;
      let r;
      if (dayName) {
        r = "Sorry, " + dayName.charAt(0).toUpperCase() + dayName.slice(1) + " is fully booked. Would you like to try a different day? You can say something like 'How about Tuesday?' or 'What's available next week?'";
      } else {
        r = "I'm not finding openings in that window. Would you like to try a specific day? Just say something like 'How about Wednesday?'";
      }
      logMessage(from, "assistant", r, null);
      return { text: r, stage: "selecting_slot", qualified: true, leadData };
    }
  ...
  // Handle slot number selection (1, 2, or 3)
  if (idx >= 0 && idx < slots.length) {
    ...
    if (result.success) {
      leadData.calendar_event_id = result.eventId;
      leadData.travel_event_id = result.travelEventId;
      leadData.assessment_time = result.startTime;
      delete leadData.available_slots;
      updateConversation(from, "assessment_booked", leadData);
      saveLead(from, leadData);
      syncLeadToHubSpot(from, leadData).catch(console.error);
      try { scheduleFollowUps(from, leadData); scheduleTextFollowUps(from, leadData); } catch(e) {}
      // Schedule day-of reminders (2 hours before assessment)
      try {
        const slotStartISO = recon[idx].start.toISOString();
        scheduleDayOfReminder(from, leadData, slotStartISO);
        scheduleDayOfEmailReminder(from, leadData, slotStartISO);
      } catch(e) { console.error("Day-of reminder scheduling:", e.message); }
  ...
  // Couldn't parse their selection — let Claude handle but strip tags from response
  const mh = buildMessageHistory(from);
  const sc = buildStateContext(conversation, { score: "neutral", override: false, summary: "" });
  const ar = await callClaude(mh, sc);
  const cleanReply = ar.replace(/\[\[LEAD_DATA:[^\]]+\]\]/g, "").replace(/\[\[QUALIFIED\]\]/g, "").trim();
  logMessage(from, "assistant", cleanReply, null);
  return { text: cleanReply, stage: "selecting_slot", qualified: true, leadData };
}
...
async function handleCancellation(from, body, conversation) {
  ...
  delete ld.calendar_event_id;
  delete ld.travel_event_id;
  delete ld.assessment_time;
  updateConversation(from, "cancelled", ld);
  try { scheduleFollowUps(from, ld); scheduleTextFollowUps(from, ld); } catch(e) {}
  logMessage(from, "assistant", r, null);
  return { text: r, stage: "cancelled", qualified: true, leadData: ld };
}

function isCancellationRequest(t) {
  const l = t.toLowerCase();
  return l.includes("cancel") || l.includes("can't make it") || l.includes("cant make it") || l.includes("need to reschedule") || l.includes("won't be able");
}

function isSlotSelection(t) {
  const l = t.trim().toLowerCase();
  // Check for decline/delay language FIRST — these are NOT slot selections
  const declinePatterns = ["no ", "no,", "no.", "not now", "not yet", "not right now", "don't want",
    "can't do", "won't work", "need to talk", "hold off", "think about it", "talk to",
    "let me think", "i'll pass", "maybe later", "not ready", "not interested"];
  for (const dp of declinePatterns) {
    if (l.includes(dp) || l === "no") return false;
  }
  return /^[1-3]$/.test(l) || l.includes("first") || l.includes("second") || l.includes("third") ||
    l.includes("one") || l.includes("two") || l.includes("three") ||
    l.includes("none") || l.includes("different") || l.includes("other day") || l.includes("other") ||
    l.includes("monday") || l.includes("tuesday") || l.includes("wednesday") ||
    l.includes("thursday") || l.includes("friday") || l.includes("next week") ||
    l.includes("later") || l.includes("another") || l.includes("what about") || l.includes("do you have");
}
...
function handleOptOut(p) {
  db.prepare("UPDATE conversations SET stage='opted_out',updated_at=datetime('now') WHERE phone=?").run(p);
  return { text: "You've been unsubscribed. Text again or call (208) 888-3611 anytime. Take care!", stage: "opted_out", qualified: false, leadData: {} };
}

async function callClaude(mh, sc) {
  try {
    const c = getClient();
    const r = await c.messages.create({ model: "claude-sonnet-4-6", max_tokens: 500, system: SYSTEM_PROMPT + "\n\n" + sc, messages: mh });
    return r.content[0].text;
  } catch(e) {
    console.error("Claude API error:", e.message);
    return "Thanks for your message! Call (208) 888-3611 for help.";
  }
}
...
function buildMessageHistory(phone) {
  ...
  for (const r of rows) {
    ...
    if (l && l.role === r.role) l.content += "\n" + r.content;
    else m.push({ role: r.role, content: r.content });
  }
...
FULL NAMES REQUIRED: Always get FIRST and LAST name for both the contact person and the care recipient. If they only give a first name, ask for their last name before qualifying.
FORMAT: Keep under 320 chars. Tag data at END: [[LEAD_DATA:field=value]]. Fields: relationship,contact_name,care_recipient_name,care_type,in_service_area,urgency,name,phone,email,insurance,referral_source,address. When qualified (full name+phone+address+NOT Medicaid+adult): [[QUALIFIED]]`;
}
...
function getStageInstructions(s, d) {
  const i = {
    ...
    insurance: "Screen insurance. Medicaid=at capacity. Medicare=no coverage. VA/LTC/private=continue.",
    urgency: "Ask timeline. Get exact address for assessment.",
    capture_info: "Get remaining: FULL name (first+last), phone, email(optional), ADDRESS. Push free assessment. Have: " + JSON.stringify(d),
    selecting_slot: "Slots offered. System handles booking. Answer other questions.",
    assessment_booked: "Booked: " + (d.assessment_time || "soon") + ". Coordinator will call. Handle cancellation warmly.",
    cancelled: "Cancelled. Ask feedback gently. Offer reschedule.",
  ...
  return i[s] || "Guide toward qualification and scheduling.";
}
...
function parseAIResponse(ai, body, conv) {
  ...
  if (nd.name && (nd.phone || nd.email) && nd.address && !q && !(nd.insurance || "").toLowerCase().includes("medicaid")) { q = true; st = "complete"; }

  return {
    text: ai.replace(/\[\[LEAD_DATA:[^\]]+\]\]/g, "").replace(/\[\[QUALIFIED\]\]/g, "").trim(),
    stage: st,
    qualified: q,
    leadData: nd,
  };
...
function getOrCreateConversation(p) {
  let c = db.prepare("SELECT * FROM conversations WHERE phone=?").get(p);
  if (!c) { db.prepare("INSERT INTO conversations(phone,stage,lead_data,message_count,created_at,updated_at)VALUES(?,'greeting','{}',0,datetime('now'),datetime('now'))").run(p); c = db.prepare("SELECT * FROM conversations WHERE phone=?").get(p); }
  if (c.stage === "opted_out") { db.prepare("UPDATE conversations SET stage='greeting',lead_data='{}',message_count=0,updated_at=datetime('now') WHERE phone=?").run(p); c = db.prepare("SELECT * FROM conversations WHERE phone=?").get(p); }
  return c;
}
...
function saveLead(p, ld) {
  const e = db.prepare("SELECT * FROM leads WHERE phone=?").get(p);
  if (e) {
    db.prepare("UPDATE leads SET name=?,email=?,care_type=?,care_recipient_name=?,relationship=?,urgency=?,insurance_type=?,referral_source=?,in_service_area=?,qualified=1,updated_at=datetime('now') WHERE phone=?")
      .run(ld.name, ld.email, ld.care_type, ld.care_recipient_name, ld.relationship, ld.urgency, ld.insurance, ld.referral_source, ld.in_service_area, p);
  } else {
    db.prepare("INSERT INTO leads(phone,name,email,care_type,care_recipient_name,relationship,urgency,insurance_type,referral_source,in_service_area,qualified,created_at,updated_at)VALUES(?,?,?,?,?,?,?,?,?,?,1,datetime('now'),datetime('now'))")
      .run(p, ld.name, ld.email, ld.care_type, ld.care_recipient_name, ld.relationship, ld.urgency, ld.insurance, ld.referral_source, ld.in_service_area);
  }
  console.log("Lead saved: " + ld.name + " (" + p + ")");
}
