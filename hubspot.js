...
export async function syncLeadToHubSpot(phone, leadData) {
  ...
  try {
    ...
    const properties = {
      ...
      // Custom properties (you'll create these in HubSpot)
    ...
    if (existing) {
      ...
      console.log(`📋 HubSpot: Updated contact ${contactId} (${leadData.name})`);
    } else {
      // Create new contact
      ...
      if (result) {
        console.log(`📋 HubSpot: Created contact ${result.id} (${leadData.name})`);
      }
    }

    // Create a note with the conversation summary
    await createEngagementNote(phone, leadData);
